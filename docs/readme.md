...
- .env som er i gitignore for å ikke havne på Github
- Miljøvariabler for db-passord, db-brukernavn, o.l.
- Parameteriserte spørringer med %s ved queries
- Validering av brukernavn om det er tatt (ikke inført enda)
- Feilhåndtering i python med try except (til å lage tabeller om ikke finnes, f.eks.)
...
**Kunne ikke pinge eller koble til Pi (brannmur):**
...
**Kunne ikke koble til Pi (DNS):**

- Problemet var med DNS-konfigurasjonen, altså innholdet i filen **/etc/resolv.conf**. Filen så slik ut:
...
Jeg vet ikke hvorfor den skrev disse addressene, men ifølge whois.com tilhører de Telenor, uansett byttet jeg de. Den nye filen ble sånn her:
...
Den første er kubens dns-server og den andre googles, og nå fungerte kobling til Pi igjen.
...
Det som var mest utfordrende var å sette opp en god struktur av databasen og hente frem den informasjon til brukersiden på nettsiden. Det var mange Foreign Keys å tenke på, og hvilke rekkefølge man må lage og slette tabeller når det gjaldt struktur. Det med å hente info til brukersiden brukte jeg en del tid på, selv om den kan forbedres herifra.
